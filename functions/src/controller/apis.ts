...
const bucket = admin.storage().bucket("nsccpccoe.appspot.com");
...
export const getPage = async (req: express.Request, res: express.Response) => {
  try {
    ...
    const likes = await admin
        ...
        .where("postID", "==", _id)
        .get();
    const likeID: Array<string> = [];
    likes.docs.map((doc) => {
      likeID.push(doc.data().userID);
    });
    if (!response.exists) {
...
export const like = async (req: express.Request, res: express.Response) => {
  try {
    ...
    const response = await admin
        ...
        .collection(collectionPath + "/likes")
        .doc(`${docID}#${uid}`)
        .create({postID: docID, userID: uid, timestamp: time});
